...
\note{Prereq: All of linear algebra basically, including page rank etc.  \\
...
\note{ 
Make sure to remind students that this only works because the matrix is already in RRE form. Getting a matrix to RRE form and then doing this same strategy is NOT a correct way to calculate eigenvalues. 
}
...
\note{Please draw a picture to show what the matrix does to a vector. Also remember we are only considering real eigenvalues, as written in the prompt of the problem.}
...
\sol{
    Remember that for upper triangular matrices, the eigenvalues can be read from the diagonal. $1, \frac{1}{2}, \frac{1}{3}$ are the three eigenvalues. This is because when multiplied by this matrix, any vector of the form $\begin{bmatrix} x \\ 0 \\ 0 \end{bmatrix}$ will be scaled by 1, $\begin{bmatrix} 0 \\ x \\ 0 \end{bmatrix}$ will be scaled by 1/2 and $\begin{bmatrix} 0 \\ 0 \\ x \end{bmatrix}$ will be scaled by 1/3.
}
