...
\note{Plot these points on a plane and show students how these points can be used to represent a signal that passes through them}
...
\begin{enumerate}
\item{Give an expression for the signal $y[k]$ that the user sees, in terms of the signals $s_i[k]$, attenuation factors $\alpha_i$, and delays $\tau_i$ (Hint: There will be a linear combination involved).}
...
\note{Explain the concept of "wrapping around" to students with an example.}

\item{First, consider the case when there is only one satellite, with a signal $s$ (which is known to the user). And assume that $y$ can consist of various shifts of $s$. That is, $y[k] = \sum_{i = 1}^{N} \beta_i s[k - i]$. Intuitively, this might happen if the signal bounces off multiple surfaces, resulting in a superposition of $s$ with different shifts and attenuations. How can the user recover the attenuation and the time shifts of the constituent $s$ signals? Formulate this problem as a system of equations.}
...
\item{Now, consider the case when there are multiple satellites. Will the same method as above work? Why/Why not?}
...
\begin{center}
  \makebox[\linewidth]{\includegraphics[width=\paperwidth]{{"../q_cross_correlation_figs/img1"}.png}}
\end{center}
...
\begin{center}
  \makebox[\linewidth]{\includegraphics[width=\paperwidth]{{"../q_cross_correlation_figs/img2"}.png}}
\end{center}
