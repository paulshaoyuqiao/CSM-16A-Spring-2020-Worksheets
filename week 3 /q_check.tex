...
\note{When walking through every statement in this question, be sure to encourage the students to think from the ground up (beginning with definitions). It is very tempting to try to fit brute-force counterexample; but in some cases, it might be very hard to directly come up with them without understanding the concept first.}
\begin{enumerate}
...
\note{
        Make sure to clarify the definition of a null space and how the dimension of the null space and the dimension of the column space of A $\in \mathbb{R}^{m \times n}$ are related to each other:
        $$ \text{dim(Col}(\mathbf{A})) + \text{dim(Nul}(\mathbf{A})) = n $$
    }
...
\end{bmatrix}$$ This matrix has only $\vec{0}$ as the solution to $$A\vec{x} = \vec{0}$$; however, since it is not square, we cannot find an inverse for it.}
    ...
    $$A^T(A^T)^{-1} = I$$
    Hence, $$A^T(A^{-1})^T = A^T(A^T)^{-1}, A^T((A^{-1})^T - (A^T)^{-1}) = 0$$
